...
class Meli
{
    ...
    /*
      get_1_item_features work different than get_items_feature
        the difference is that it is born from the entrance item_id not the api object, searches the objects obtained from the different apis their characteristics
    */   

    public function item_features_result ($params) {
    ...
    $item_features_result = $this -> get_1_item_features($item_id_input, $features_to_obtain = null, $country_id="MLM", $coin="MLM", $fast_mode = true);
    ...
    return $item_features_result;
  }   

    public function get_1_item_features($item_id, $features_to_obtain, $country_id="MLM", $coin="MLM", $fast_mode = false){

      //url objeto id
      $first_urls[0] = self::$GET_FULL_ARTICLE_URL . $item_id; 
      $first_urls[1] = "https://api.mercadolibre.com/visits/items?ids=" . $item_id;
      $first_urls[2] = "https://api.mercadolibre.com/reviews/item/" . $item_id;

      // llamar todas las url primer round
      $response_first_round = curl::request_multiple($first_urls, $options = array(CURLOPT_SSL_VERIFYPEER => false));

      //llamar object_item_id
      $object_item_id = json_decode($response_first_round[0], $assoc = true);
      $object_array[0] = $object_item_id;
      $views_array[0] = json_decode($response_first_round[1], $assoc = true);
      $object_reviews = json_decode($response_first_round[2], $assoc = true);

      //sacar datos de ahí
      $item_features["title"] = $object_item_id["title"];
      $item_features["site_id"] = $object_item_id["site_id"];
      // Pendiente verrificar como se comporta en mexico con dolares
      $item_features["price"] = $object_item_id["price"];

      $item_features["logistic_type"] = $object_item_id["shipping"]["logistic_type"];
      $item_features["free_shipping"] = $object_item_id["shipping"]["free_shipping"];
      $item_features["condition"] = $object_item_id["condition"];
      if(isset($object_item_id["catalog_product_id"])){$item_features["catalog_product"] = true;}
      else{$item_features["catalog_product"] = false;}
      if(isset($object_item_id["video"])){$item_features["video"] = true;}
      else{$item_features["video"] = false;}   
      $item_features["accepts_mercadopago"] = $object_item_id["accepts_mercadopago"];
      $item_features["tags"] = implode(" ", $object_item_id["tags"]);
      $item_features["num_pictures"] = count($object_item_id["pictures"]);
      $attributes_array = $this -> string_attribute($object_array);
      $item_features["attributes"] = $attributes_array[0];
      $item_features["reviews_average"] = $object_reviews["rating_average"]; 
      $item_features["reviews_total"] = $object_reviews["paging"]["total"];       
      if(is_null($object_item_id["official_store_id"])){$item_features["official_store"] = true;}
      else{$item_features["official_store"] = false;} 
      if(empty($object_item_id["deal_ids"])){$item_features["deal_ids"] = false;}
      else{$item_features["deal_ids"] = true;} 
      if(is_null($object_item_id["warranty"])){$item_features["warranty"] = false;}
      else{$item_features["warranty"] = true;}
      $item_features["listing_type_id"] = $object_item_id["listing_type_id"]; 
      $item_features["category_id"] = $object_item_id["category_id"];    
       
      //conversion
      
          //sold_quantity
            $sold_quantity_array[0] = $object_item_id["sold_quantity"];
          //views 
        ...
        foreach ($object_array as $key => $value) {
          ...
          $item_features["conversion"] = $var1/$var2;

          //Pendiente pasar a funcion covertir a 0 float
          
          if ((is_nan($item_features["conversion"])) || (is_infinite($item_features["conversion"])) || ($item_features["conversion"] === 0)) {
            $item_features["conversion"] = 0;
            $item_features["conversion"] = floatval($item_features["conversion"]);
          }
      ...
      if ($fast_mode == true) {
        ...
        //unir todas las llamadas a un segundo round
        //1- llamar a api seller
        //2- llamar a api categoria para saber cantidad de articulos en esta
        //3- llamar a las 50 llamadas del identificador de ranking.

        $second_urls[0] = "https://api.mercadolibre.com/users/" . $object_item_id["seller_id"]; 
        $second_urls[1] = "https://api.mercadolibre.com/categories/" . $object_item_id["category_id"];

        //var_dump("second_urls");
        //var_dump($second_urls);

        $response_second_round = curl::request_multiple($second_urls, $options = array(CURLOPT_SSL_VERIFYPEER => false));

        foreach ($response_second_round as $key => $value) {
            $response_second_round[$key] = json_decode($response_second_round[$key], $assoc = true);
        }

        //users
        $object_users[0] = $response_second_round[0];
        $reputation_array[0] = $object_users[0]['seller_reputation']['level_id'];
        $portions = explode("_", $reputation_array[0]);
        $item_features["reputation_vendor"] = intval($portions[0]);

        if ($object_users[0]["seller_reputation"]["transactions"]["completed"] === null) {
          $item_features["vendor_sales_completed"] = ""; 
        }else{
          $item_features["vendor_sales_completed"] = $object_users[0]["seller_reputation"]["transactions"]["completed"];
        }

        //ranking
        $total_items_in_this_category = $response_second_round[1]["total_items_in_this_category"];
        if($total_items_in_this_category > 1000){$total_articles = 1050;}
        else{$total_articles = $total_items_in_this_category;}

        $item_id_array[0] = $item_id;
        $category_id =  $object_item_id["category_id"];
        $total_articles = 1050; //en orden de ver si esta en los primeros 1050 sino se pasa a logica con objetivo de devolver resultado rapidamente
        $ranking_position = $this -> get_ranking_item_less1000($item_id_array, $category_id, $total_articles, $limit = 50, $country_id);
        //var_dump("get_ranking_item result");
        //var_dump($ranking_position);
        //var_dump(gettype($ranking_position[0]));
        if (($ranking_position[0] == null) || empty($ranking_position[0])) {
            $ranking_position[0] = ""; 
          }
        $item_features["ranking"] = $ranking_position[0];



        /*
        //users
        $seller_id_array[0] = $object_item_id["seller_id"];
        $object_users = analysis::get_seller($seller_id_array);
        ...
        //agregar second round
        //var_dump("fast_mode == true");
        $item_id_array[0] = $item_id;
        $category_id =  $object_item_id["category_id"];
        $total_articles = 1050; //en orden de ver si esta en los primeros 1050 sino se pasa a logica con objetivo de devolver resultado rapidamente
        $ranking_position = $this -> get_ranking_item_less1000($item_id_array, $category_id, $total_articles, $limit = 50, $country_id);
        ...
        */
        ...
        $item_id_array[0] = $item_id;
        $category_id =  $object_item_id["category_id"];   
        $obj = $this -> get_ranking_item($item_id_array, $category_id, $limit = 50, $country_id);

        //var_dump("get_ranking_item result");
        //var_dump($obj);

        $item_features["ranking"] = $obj[0];  
      }
    ...
    */ 
    ...
    public function data_extractor($params){
      ...
      $categories_ids_string = $portions[1];

      // "%2C" is "," http languaje translation
      $category_array = explode("%2C", $categories_ids_string);

      var_dump("$category_array");
      var_dump($category_array);

      $cousin_and_total_items = $this -> comparison_model_prepare_to_getfeatures_v4($category_array, $plus= null, $force_calculation_under_1000 = true, $country_base = "MLM");

      $this -> get_items_features_unified_v4($cousin_and_total_items["category_cousins"], $country_base = "MLM");
      ...
      return "TODO respuesta correcta";
    }
    ...
    public function get_items_features_unified_v3($min_children_category, $max_children_category, $category_array, $country_base){
     ...
     // Data data_extractor solds_per_day (version 3)  //

    //Pendiente copiar de la base de api la funcion "data_extractor" aqui, con el objetivo de tenerlas registrada como disparador de esta version 3

    public function comparison_model_prepare_to_getfeatures_v4($category_array, $plus, $force_calculation_under_1000, $country_base){

      //return to v_2 concept

      //Diference vs v_2 
      //1- count with data from other countries, just $country_base

      //Diference vs v_3 
      //1- for children_categories not fathers_of_children_categories
      //2- entry is just $category_array ej: ("0" => "MLM158032", "1" => "MLM126075")
    ...
    public function get_items_features_unified_v4($category_cousins, $country_base){


      //return to v_2 concept

      //Diference vs v_2 
      //1- receive data from other countries, just $country_base

      //Diference vs v_3 
      //1- not $min_children_category, $max_children_category
