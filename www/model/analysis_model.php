...
class Analysis extends Meli
{
    ...
    */                          
    ...
    public static function item_features_object_mode_ML($item_features){

        foreach ($item_features as $key => $value) {
            if (is_bool($item_features[$key]) === true) {
               $item_features[$key] = $item_features[$key] ? 'true' : 'false';
               //$item_features[$key] = "" . $item_features[$key]; 
            }
        }
